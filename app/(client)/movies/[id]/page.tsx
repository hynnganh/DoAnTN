...
import { Play, Star, Heart, Award, Calendar, Globe, Film, Ticket, Loader2 } from 'lucide-react';
...
export default function MovieDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!movie) return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center gap-4">
      <h2 className="uppercase font-black italic">Không tìm thấy phim</h2>
      <Link href="/" className="text-xs border-b border-zinc-800 text-zinc-500">Quay về trang chủ</Link>
    </div>
  );
      ...
      <section className="relative h-[70vh] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center pt-20">
          <div className="max-w-6xl mx-auto px-6 w-full flex flex-col md:flex-row gap-10 items-center md:items-end pb-10">
            <div className="w-44 md:w-64 aspect-[2/3] rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/10 shrink-0">
              <img src={movie.posterUrl} className="w-full h-full object-cover" alt="poster" />
            </div>

            <div className="flex-1 space-y-6 text-center md:text-left">
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                <span className="bg-red-600 text-white font-[1000] px-3 py-1.5 rounded-xl text-[9px] uppercase tracking-widest italic shadow-lg shadow-red-600/20">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-center md:justify-start gap-4">
                   ...
                   <p className="text-[10px] font-black uppercase text-zinc-500 italic tracking-widest">{movie.genre?.name}</p>
                </div>
              </div>

              <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                {movie.status === "SHOWING" && (
                  <Link href={`/movie/${movieId}/booking/`} className="flex items-center gap-3 px-8 py-4 bg-red-600 text-white rounded-[1.5rem] text-[11px] font-[1000] uppercase tracking-widest hover:bg-white hover:text-black transition-all active:scale-95 shadow-xl shadow-red-600/20">
                    <Ticket size={16} /> ĐẶT VÉ NGAY
                  </Link>
                )}
                <button onClick={() => setIsLiked(!isLiked)} className={`p-4 rounded-[1.5rem] border transition-all active:scale-90 ${isLiked ? 'bg-red-600 border-red-600 text-white' : 'bg-white/5 border-white/10 text-zinc-500'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20">
          <div className="lg:col-span-8 space-y-12">
            <div className="space-y-6">
              <h3 className="text-[12px] font-[1000] uppercase tracking-[0.4em] text-red-600 italic flex items-center gap-4">
                <span className="w-10 h-[3px] bg-red-600 rounded-full" /> NỘI DUNG PHIM
              </h3>
              <p className="text-zinc-400 text-lg md:text-xl leading-[1.8] font-medium italic bg-zinc-900/10 p-8 rounded-[3rem] border border-white/5 shadow-inner">
                {movie.description}
              </p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-8 bg-zinc-900/20 rounded-[3rem] border border-white/5 text-center">
          ...
          <div className="lg:col-span-4">
             <div className="p-8 bg-zinc-950 border border-white/5 rounded-[3.5rem] space-y-8 sticky top-10 shadow-2xl">
                <div className="space-y-4 text-center border-b border-white/5 pb-8">
                  ...
                  <p className="text-xs text-zinc-500 font-bold italic">Bạn đã xem phim? Để lại cảm nhận cho mọi người nhé!</p>
                </div>
...
function InfoBox({ icon, label, value }: { icon: any, label: string, value: string }) {
  return (
    <div className="space-y-2">
      <div className="text-red-600 flex justify-center">{icon}</div>
